...
def train(train_dataset):
    ...
    # 6. 保存模型.
    torch.save(model.state_dict(), '../../models/last_image_model.pth')
...
def evaluate(test_dataset):
    ...
    model = ImageModel()
    # 3. 加载模型参数.
    model.load_state_dict(torch.load('../../models/last_image_model.pth'))    # pickle文件
...
if __name__ == '__main__':
    ...
    # 查看模型参数，此时模型与输入将位于同一设备
    summary(model, (3, 32, 32), batch_size=1)

    # 3. 模型训练（如果希望训练也在 GPU 上，需要额外修改 train 函数，见下方说明）
    # train(train_dataset)
